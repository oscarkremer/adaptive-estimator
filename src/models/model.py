...
class Estimator:
    ...
    def train(self, t, u, y, lambda_forg=1):
        error, theta_plot = [], []
        self.P = self.rastreabilty*np.identity(4)
        ...
        for i in range(t.shape[0]):
            if np.round(t[i], 3) in time_list:
                self.P = self.rastreabilty*np.identity(4)
            error.append(abs(y[i] - theta[0]*u[0][i] - theta[1]*u[1][i] - theta[2]*u[2][i] - theta[3]*u[3][i])/y[i])
            p[i] = np.linalg.norm(self.P, ord='fro')
            norm[i] = p[i]/np.linalg.norm(self.rastreabilty*np.identity(4), ord='fro')
            theta_plot.append(theta)
            fi = np.array([[u[0][i]], [u[1][i]], [u[2][i]], [u[3][i]]])
            K = self.P.dot(fi)/(lambda_forg + (np.transpose(fi).dot(self.P)).dot(fi))
            self.P = (np.identity(4) - K.dot(np.transpose(fi))).dot(self.P)/lambda_forg
            theta = theta + K.dot(y[i] - np.transpose(fi).dot(theta))
            y_hat[i] = theta_plot[i][0]*u[0][i] + theta_plot[i][1]*u[1][i] + theta_plot[i][2]*u[2][i] + theta_plot[i][3]*u[3][i]
        self.y_hat = y_hat
        self.error = error
